...
class TowerDefenseGame:
    ...
    def pause_menu(self):
        paused=True; font_large=pygame.font.SysFont(None,72); font_btn=pygame.font.SysFont(None,48)
        ...
        while paused:
            ...
            t=font_large.render("PAUSED",True,config.WHITE); self.screen.blit(t,t.get_rect(center=(SCREEN_WIDTH//2,150)))
            mpos=pygame.mouse.get_pos();
            for b in [resume_btn,exit_btn]: b.changeColor(mpos); b.update(self.screen)
            pygame.display.flip()
            for ev in pygame.event.get():
    ...
    def game_over_screen(self):
        font_large=pygame.font.SysFont(None,72); font_btn=pygame.font.SysFont(None,48)
        ...
        while True:
            ...
            t=font_large.render("GAME OVER",True,config.RED); self.screen.blit(t,t.get_rect(center=(SCREEN_WIDTH//2,150)))
            mpos=pygame.mouse.get_pos();
            for b in [restart_btn,exit_btn]: b.changeColor(mpos); b.update(self.screen)
            pygame.display.flip()
            for ev in pygame.event.get():
                if ev.type==pygame.QUIT: pygame.quit(); sys.exit()
                if ev.type==pygame.MOUSEBUTTONDOWN:
                    if restart_btn.checkForInput(mpos): return
                    elif exit_btn.checkForInput(mpos): self.running=False; return
